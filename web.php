...
<style>#nav{position: fixed; top: 0; left: 30%;}</style>
</head>
<body background="f.jpg" bgcolor=efefef link=red vlink=red><center><div id="nav"><h3><span style="color:gray">Web based, DYNAMIC <span style="color:#9c353a">PORT SCANNER</span></span></h3>
<form method="post">
...
elseif (strlen($endport) < 1) {echo "";exit;}
...
elseif(stripos( $domain, 'mehvar.xyz' ) !== false) {echo "This site cannot be port-scanned";exit;}
elseif(strpos( $domain, '.' ) != true){echo"where is the dot";exit;}
elseif(strpos( $domain, ';' ) !== false) {echo "Bad Input! Try again";exit;}
...
if($startport > $endport)
	...
	$control = abs($limit);
	if($control > 101) {echo "Bulk port scanning barred due to bandwidth issues. Max range can be 101";exit;}
	echo 'Scan started for: <b>' . abs($limit) . '</b> ports of:<b> ' . $domain . '</b> from Port: <b>' . $startport . '</b> to Port: <b>' . $endport . '</b><br /><br />';
